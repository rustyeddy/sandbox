...
require_once( WEBMAN_LIB_DIR . 'functions.php' );

/*
 * Sites are custom post types 'webman_site', each site
...
class WEBMAN_Site
{
	...
	$this->post_meta = array(
	    ...
	    '_webman_form_id' => null,
	    '_webman_entry_id' => null,

	    '_webman_has_dropbox' => false,
	    '_webman_dropbox_account' => null,
			     
	    '_webman_sandbox' => null,
	    '_webman_status' => 'sandbox',

	    '_webman_theme_provider' => 'genesis',
	    '_webman_theme_name' => '',	
	    );
    }
	...
	if ( ! array_key_exists( $form_id, $webman_forms_map ) ) {

	    webman_logit ( "Could not find form id: " . $form_id .
			" will not be able to create site post");

	    return null;
	}

	$map = $webman_forms_map[ $form_id ];
    ...
    {
	$html = webman_table_from_array( $this->post_meta );
	return $html;
    }
 ...
 */
add_action( 'init', 'webman_create_site_post_type' );
function webman_create_site_post_type( )
{
    ...
    $ret = register_post_type( 'webman_site', $args );
...
function webman_deactivate_site_post_type( )
{
    flush_rewrite_rules();
}
 ...
 * Add the site list
 */
add_shortcode( 'sites', 'webman_sites');
function webman_get_site_loop()
{
    global $post;

    // pull up all webman_sites
    $args = array(
		'post_type'	=> '_webman_sites',
	);
