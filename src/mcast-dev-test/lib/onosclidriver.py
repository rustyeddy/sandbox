...
from etc.config import LINUX_PROMPT
    ...
    def connect(self, **connectargs):
        ...
        try:
            ...
            try:
                ...
                self.logger.info("Invalid host name," +
                                 " connecting to local host instead")
                self.ip_address = 'localhost'
            except Exception as inst:
                self.logger.error("Uncaught exception: " + str(inst))
            ...
            self.handle.expect(LINUX_PROMPT)
            if self.handle:
                return self.handle
            else:
                self.logger.info("NO ONOS HANDLE")
                return False
        except TypeError:
...
#           main.exit()
    ...
    def disconnect(self):
        ...
        try:
            ...
            self.logger.exception("Exception in disconnect of " + self.name)
            response = True
        except Exception:
            self.logger.exception(
                self.name + ": Connection failed to the host")
            response = False
        self.logger.debug("Closed ONOS CLI interface.")
        return response

    def logout(self):
        """
        Sends 'logout' command to ONOS cli
        Returns True if exited CLI and
                False on timeout (not guranteed you are disconnected)
                None on TypeError
        ...
        try:
            if self.handle:
                self.handle.sendline("")
                i = self.handle.expect(["onos>", LINUX_PROMPT, pexpect.TIMEOUT],
                                       timeout=10)
                if i == 0:  # In ONOS CLI
...
#           main.exit()
    ...
    def setCell(self, cellname):
        ...
        try:
            if not cellname:
                self.logger.error("Must define cellname")
    #           main.cleanup()
    #           main.exit()
            else:
                ...
                self.logger.info("Cell call returned: " + handleBefore +
    ...
    def startOnosCli(self, ONOSIp, karafTimeout="",
        ...
        try:
            ...
            x = self.handle.expect([
                LINUX_PROMPT, "onos>"], commandlineTimeout)
            ...
            if i == 0:
                self.logger.info(str(ONOSIp) + " CLI Started successfully")
                if karafTimeout:
                    self.handle.sendline(
                ...
                i = self.handle.expect(["onos>", pexpect.TIMEOUT],
                                       timeout=30)
                if i == 0:
                    self.logger.info(str(ONOSIp) + " CLI Started " +
                    ...
                    self.logger.error("Connection to CLI " +
                                      str(ONOSIp) + " timeout")
                    return False
...
#           main.exit()
    ...
    def sendline(self, cmdStr, debug=False):
        ...
        try:
            ...
            if i == 2:
                self.handle.sendline()
                self.handle.expect([LINUX_PROMPT, pexpect.TIMEOUT])
                response += self.handle.before
                print response
                try:
    ...
    def addNode(self, nodeId, ONOSIp, tcpPort=""):
        ...
        try:
            ...
            if re.search("Error", handle):
                self.logger.error("Error in adding node")
                self.logger.error(handle)
                return False
            else:
                self.logger.info("Node " + str(ONOSIp) + " added")
                return True
        except TypeError:
            self.logger.exception(self.name + ": Object not as expected")
            return None
...
#           main.exit()
    ...
    def removeNode(self, nodeId):
        ...
        try:
            ...
            handle = self.sendline(cmdStr)
            if re.search("Error", handle):
                self.logger.error("Error in removing node")
                self.logger.error(handle)
                return False
            else:
                return True
...
#           main.exit()
    ...
    def topology(self):
        ...
        try:
            cmdStr = "topology -j"
            handle = self.sendline(cmdStr)
            self.logger.info(cmdStr + " returned: " + str(handle))
            return handle
        except TypeError:
            self.logger.exception(self.name + ": Object not as expected")
            return None
...
#           main.exit()
    ...
    def featureInstall(self, featureStr):
        ...
        try:
            ...
            handle = self.sendline(cmdStr)
            if re.search("Error", handle):
                self.logger.error("Error in installing feature")
                self.logger.error(handle)
                return False
            else:
                return True
            ...
            self.logger.error(self.name + ": EOF exception found")
            self.logger.error(self.name + ":    " + self.handle.before)
            self.logger.report("Failed to install feature")
            self.logger.report("Exiting test")
#           main.cleanup()
#           main.exit()
        except Exception:
            self.logger.exception(self.name + ": Uncaught exception!")
    ...
    def featureUninstall(self, featureStr):
        ...
        try:
            ...
            if handle != '':
                ...
                self.logger.info("Feature needs to be installed before " +
                                 "uninstalling it")
                return True
            if re.search("Error", output):
                self.logger.error("Error in uninstalling feature")
...
#           main.exit()
    ...
    def deviceRemove(self, deviceId):
        ...
        try:
            ...
            handle = self.sendline(cmdStr)
            if re.search("Error", handle):
                self.logger.error("Error in removing device")
                self.logger.error(handle)
                return False
            else:
                return True
...
#           main.exit()
    ...
    def balanceMasters(self):
        ...
        try:
            ...
            handle = self.sendline(cmdStr)
            if re.search("Error", handle):
                self.logger.error("Error in balancing masters")
                self.logger.error(handle)
                return False
            else:
                return True
...
#           main.exit()
    ...
    def checkBalanceMasters(self, jsonFormat=True):
        """
            ...
            Returns True if the devices are balanced
            Returns False if the devices are unbalanced
            Exits on Exception
        ...
        try:
            ...
            for master in masters:
                ...
                if master["size"] > first + 1 or master["size"] < first - 1:
                    self.logger.error("Mastership not balanced")
                    self.logger.info("\n" + self.checkMasters(False))
                    return False
            self.logger.info("Mastership balanced between "
...
#           main.exit()
    ...
    def rolesNotNull(self):
        ...
        Returns: True if each device has a master
                 False any device has no master
        """
        try:
            rawRoles = self.roles()
            rolesJson = json.loads(rawRoles)
            ...
            for device in rolesJson:
                ...
                if device['master'] == "none":
                    self.logger.warn("Device has no master: " + str(device))
                    return False
            return True
...
#           main.exit()
    ...
    def paths(self, srcId, dstId):
        ...
        try:
            ...
            handle = self.sendline(cmdStr)
            if re.search("Error", handle):
                self.logger.error("Error in getting paths")
                return (handle, "Error")
            else:
                path = handle.split(";")[0]
                cost = handle.split(";")[1]
...
#           main.exit()
    ...
    def addHostIntent(self, hostIdOne, hostIdTwo):
        ...
        try:
            ...
            handle = self.sendline(cmdStr)
            if re.search("Error", handle):
                self.logger.error("Error in adding Host intent")
                self.logger.debug("Response from ONOS was: " + repr(handle))
                return None
            else:
                self.logger.info("Host intent installed between " +
                                 str(hostIdOne) + " and " + str(hostIdTwo))
                match = re.search('id=0x([\da-f]+),', handle)
                if match:
                    return match.group()[3:-1]
                else:
                    self.logger.error("Error, intent ID not found")
                    self.logger.debug("Response from ONOS was: " +
                                      repr(handle))
                    return None
        except TypeError:
            self.logger.exception(self.name + ": Object not as expected")
...
#           main.exit()
    ...
    def addOpticalIntent(self, ingressDevice, egressDevice):
        ...
        try:
            ...
            if re.search("Error", handle):
                self.logger.error("Error in adding Optical intent")
                return None
            else:
                self.logger.info("Optical intent installed between " +
    ...
    def addPointIntent(
        ...
        try:
            ...
            if "/" in ingressDevice:
                ...
                if not portIngress:
                    self.logger.error("You must specify the ingress port")
            ...
            if "/" in egressDevice:
                ...
                if not portEgress:
                    self.logger.error("You must specify the egress port")
                    return None
            ...
            # If error, return error message
            if re.search("Error", handle):
                self.logger.error("Error in adding point-to-point intent")
                return None
            else:
                # TODO: print out all the options in this message?
                self.logger.info("Point-to-point intent installed between " +
                                 str(ingressDevice) + " and " +
                                 str(egressDevice))
                match = re.search('id=0x([\da-f]+),', handle)
                if match:
...
#           main.exit()
    ...
    def addMultipointToSinglepointIntent(
        ...
        try:
            ...
            if portIngressList is None:
                for ingressDevice in ingressDeviceList:
                    if "/" in ingressDevice:
                        ...
                        self.logger.error("You must specify " +
                                          "the ingress port")
                        # TODO: perhaps more meaningful return
                        return False
            else:
                if len(ingressDeviceList) == len(portIngressList):
                    ...
                    self.logger.error("Device list and port list does not " +
            ...
            if re.search("Error", handle):
                self.logger.error(
                    "Error in adding multipoint-to-singlepoint " + "intent")
                return None
            else:
                match = re.search('id=0x([\da-f]+),', handle)
                if match:
...
#           main.exit()
    ...
    def addSinglepointToMultipointIntent(
        ...
        try:
            ...
            if re.search("Error", handle):
                self.logger.error(
                    "Error in adding singlepoint-to-multipoint " + "intent")
                return None
            else:
                match = re.search('id=0x([\da-f]+),', handle)
                if match:
...
#           main.exit()
    ...
    def addMplsIntent(
        ...
        try:
            ...
            # If error, return error message
            if re.search("Error", handle):
                self.logger.error("Error in adding mpls intent")
                return None
            else:
                # TODO: print out all the options in this message?
                self.logger.info("MPLS intent installed between " +
                                 str(ingressDevice) + " and " +
                                 str(egressDevice))
                match = re.search('id=0x([\da-f]+),', handle)
                if match:
...
#           main.exit()
    ...
    def removeIntent(self, intentId, app='org.onosproject.cli',
        ...
        try:
            ...
            handle = self.sendline(cmdStr)
            if re.search("Error", handle):
                self.logger.error("Error in removing intent")
                return False
            else:
...
#           main.exit()
    ...
    def purgeWithdrawnIntents(self):
        ...
        try:
            ...
            handle = self.sendline(cmdStr)
            if re.search("Error", handle):
                self.logger.error("Error in purging intents")
                return False
            else:
                return True
        except TypeError:
        ...
        try:
            ...
            if jsonFormat and summary and (type != ""):
                ...
                if type in jsonResult.keys():
                    ...
                    self.logger.error(
                        "unknown TYPE, return all types of intents")
                    return handle
            else:
                return handle
            ...
            if isinstance(intentsId, types.StringType):
                for intent in intentsJsonTemp:
                    if intentsId == intent['id']:
                        state = intent['state']
                        return state
                self.logger.info("Cannot find intent ID" + str(intentsId) +
                                 " on the list")
                return state
                ...
                if len(intentsId) != len(dictList):
                    self.logger.info("Cannot find some of the intent ID state")
                return dictList
            else:
                self.logger.info("Invalid intents ID entry")
                return None
        except TypeError:
            self.logger.exception(self.name + ": Object not as expected")
            return None
    ...
    def checkIntentState(self, intentsId, expectedState='INSTALLED'):
        ...
        try:
            ...
            if len(intentsId) != len(intentsDict):
                self.logger.info(self.name + "There is something wrong " +
            ...
            if isinstance(expectedState, types.StringType):
                for intents in intentsDict:
                    if intents.get('state') != expectedState:
                        self.logger.debug(self.name + " : Intent ID - " +
                                          intents.get('id') +
                                          " actual state = " +
                                          intents.get('state')
            ...
            if returnValue:
                self.logger.info(self.name + ": All " +
                                 str(len(intentsDict)) +
...
#           main.exit()
    ...
    def flows(self, jsonFormat=True):
        ...
        try:
            ...
            handle = self.sendline(cmdStr)
            if re.search("Error:", handle):
                self.logger.error(self.name + ": flows() response: " +
                                  str(handle))
            return handle
        except TypeError:
            self.logger.exception(self.name + ": Object not as expected")
    ...
    def checkFlowsState(self):
        ...
        Return:
            returnValue - Returns True only if all flows are in
                          ADDED state or PENDING_ADD, return False
                          otherwise.
        """
        try:
            ...
            for device in tempFlows:
                for flow in device.get('flows'):
                    if flow.get('state') != 'ADDED' and flow.get(
                            'state') != 'PENDING_ADD':

                        self.logger.info(self.name + ": flow Id: " +
...
#           main.exit()
    ...
    def getAllDevicesId(self):
        ...
        try:
            ...
            if not devicesStr:
                self.logger.info("There are no devices to get id from")
                return idList
...
#           main.exit()
    ...
    def getAllNodesId(self):
        ...
        try:
            ...
            if not nodesStr:
                self.logger.info("There are no nodes to get id from")
                return idList
...
#           main.exit()
    ...
    def checkStatus(self, ip, numoswitch, numolink, logLevel="info"):
        ...
        supplied values. By default this will report to self.logger, but the
...
#           main.exit()
    ...
    def deviceRole(self, deviceId, onosNode, role="master"):
        ...
        Returns:
            True or False based on argument verification and
            -1 if command returns and error
        """
        try:
            if role.lower() == "master" or role.lower() == "standby" or\
                ...
                self.logger.error("Invalid 'role' given to device_role(). " +
                                  "Value was '" + str(role) + "'.")
                return False
        except TypeError:
            self.logger.exception(self.name + ": Object not as expected")
    ...
    def electionTestLeader(self):
        ...
        try:
            ...
            if nodeSearch:
                ...
                self.logger.info("Election-test-leader on " + str(self.name) +
            ...
            if nullSearch:
                self.logger.info("Election-test-leader found no leader on " +
                                 self.name)
                return None
            # error
            errorPattern = "Command\snot\sfound"
            if re.search(errorPattern, response):
                self.logger.error("Election app is not loaded on " + self.name)
                # TODO: Should this be -1?
                return False
            else:
                self.logger.error(
                    "Error in electionTestLeader on " +
                    self.name +
                    ": " +
                    "unexpected response")
    ...
    def electionTestRun(self):
        ...
        try:
            ...
            if re.search(errorPattern, response):
                ...
                self.logger.error("Error in electionTestRun on " + self.name +
                                  ": " + "unexpected response")
                self.logger.error(repr(response))
                return False
        except TypeError:
    ...
    def electionTestWithdraw(self):
        ...
        try:
            ...
            if re.search(successPattern, response):
                self.logger.info(self.name + " withdrawing from leadership " +
            ...
            if re.search(errorPattern, response):
                ...
                self.logger.error("Error in electionTestWithdraw on " +
    ...
    def getDevicePortsEnabledCount(self, dpid):
        ...
        try:
            ...
            if re.search("No such device", output):
                self.logger.error("Error in getting ports")
                return (output, "Error")
            else:
                return output
        except TypeError:
...
#           main.exit()
    ...
    def apps(self, jsonFormat=True):
        ...
        # FIXME: look at specific exceptions/Errors
            ...
            self.logger.error("Error in processing onos:app command: " +
                              str(output))
            return None
        except TypeError:
            self.logger.exception(self.name + ": Object not as expected")
    ...
    def appStatus(self, appName):
        ...
        try:
            if not isinstance(appName, types.StringType):
                self.logger.error(self.name + ".appStatus(): appName must be" +
                                  " a string")
                return None
            ...
            if state == "ACTIVE" or state == "INSTALLED":
                ...
                self.logger.error("Unexpected state from 'onos:apps': " +
                                  str(state))
                return state
        except TypeError:
            self.logger.exception(self.name + ": Object not as expected")
...
#           main.exit()
    ...
    def app(self, appName, option):
        ...
        try:
            ...
            valid = True
            if not isinstance(appName, types.StringType):
                self.logger.error(self.name + ".app(): appName must be a " +
                                  "string")
                valid = False
            if not isinstance(option, types.StringType):
                self.logger.error(
                    self.name + ".app(): option must be a string")
                valid = False
            if not valid:
                return False
            # Validate Option
            ...
            if option == "activate":
                ...
                # Invalid option
                self.logger.error(
                    "The ONOS app command argument only takes " +
            ...
            if "Error executing command" in output:
                ...
                self.logger.error("The application '" + appName +
                ...
                self.logger.error("Incorrect command given to 'app': " +
                                  str(output))
            # NOTE: we may need to add more checks here
            # else: Command was successful
            # self.logger.debug( "app response: " + repr( output ) )
            return True
        except TypeError:
            self.logger.exception(self.name + ": Object not as expected")
            return -1
...
#           main.exit()
    ...
    def activateApp(self, appName, check=True):
        ...
        Returns True if the command was successfully sent
                False if the cli responded with an error or given
                    incorrect input
        """
        try:
            if not isinstance(appName, types.StringType):
            ...
            if status == "INSTALLED":
                ...
                if check and response:
                    for i in range(10):  # try 10 times then give up
                        ...
                        if status == "ACTIVE":
                            ...
                            self.logger.debug("The state of application " +
                                              appName + " is " + status)
                            time.sleep(1)
                    return False
                ...
                self.logger.error(
                    self.name +
                    ": Tried to activate the " +
                    "application '" +
                    appName +
                    "' which is not " +
                    "installed.")
            else:
                self.logger.error("Unexpected return value from appStatus: " +
                                  str(status))
                return -1
        except TypeError:
            self.logger.exception(self.name + ": Object not as expected")
    ...
    def uninstallApp(self, appName, check=True):
        ...
        try:
            ...
            if status == "INSTALLED":
                ...
                self.logger.warn(self.name + ": Tried to uninstall the " +
...
#           main.exit()
    ...
    def appIDs(self, jsonFormat=True):
        ...
        try:
            ...
            self.logger.error("Error in processing onos:app-ids command: " +
                              str(output))
            return None
        except TypeError:
            self.logger.exception(self.name + ": Object not as expected")
...
#           main.exit()
    ...
    def appToIDCheck(self):
        ...
        Returns: True  if the check passes and
                 False if the check fails or
                 -1 if there is some error in processing the test
        """
        try:
            bail = False
            ...
            if ids:
                ...
                self.logger.error("app-ids returned nothing:" + repr(ids))
                bail = True
            apps = self.apps(jsonFormat=True)
            if apps:
                apps = json.loads(apps)
            else:
                self.logger.error("apps returned nothing:" + repr(apps))
                bail = True
            if bail:
                return False
            result = True
            for app in apps:
                appID = app.get('id')
                if appID is None:
                    self.logger.error("Error parsing app: " + str(app))
                    result = False
                ...
                # self.logger.debug( "Comparing " + str( app ) + " to " +
                #                 str( current ) )
                if not current:  # if ids doesn't have this id
                    result = False
                    self.logger.error("'app-ids' does not have the ID for " +
                    ...
                    result = False
                    self.logger.error("'app-ids' has " + str(currentName) +
            ...
            if len( idsList ) != len( set( idsList ) ) or\
               len(namesList) != len(set(namesList)):
                self.logger.error("'app-ids' has some duplicate entries: \n"
                                  + json.dumps(ids,
                                               sort_keys=True,
                                               indent=4,
                                               separators=(',', ': ')))
...
#           main.exit()
    ...
    def getCfg(self, component=None, propName=None, short=False,
        ...
        try:
            ...
            self.logger.error("Error in processing 'cfg get' command: " +
                              str(output))
            return None
        except TypeError:
            self.logger.exception(self.name + ": Object not as expected")
...
#           main.exit()
    ...
    def setCfg(self, component, propName, value=None, check=True):
        ...
        returns:
            True on success or False on failure. If check is False,
            will return True unless there is an error
        """
        try:
            baseStr = "cfg"
            ...
            if value and check:
                ...
                try:
                    ...
                    self.logger.exception("Error parsing cfg output")
                    self.logger.error("output:" + repr(results))
                    return False
            ...
            self.logger.error("Error in processing 'cfg set' command: " +
                              str(output))
            return False
        except TypeError:
            self.logger.exception(self.name + ": Object not as expected")
...
#           main.exit()
    ...
    def setTestAdd(self, setName, values):
        ...
        returns:
            True on success OR
            False if elements were already in the set OR
            -1 on error
        """
        try:
            ...
            try:
                ...
                self.logger.error("Error in processing '" + cmdStr + "' " +
                                  "command: " + str(output))
                retryTime = 30  # Conservative time, given by Madan
                self.logger.info("Waiting " + str(retryTime) +
            ...
            if re.search(positiveMatch, output):
                ...
                self.logger.error(self.name + ": setTestAdd did not" +
                                  " match expected output")
                self.logger.debug(self.name + " actual: " + repr(output))
                return -1
        except AssertionError:
            self.logger.error(
                "Error in processing '" +
...
#           main.exit()
    ...
    def setTestRemove(self, setName, values, clear=False, retain=False):
        ...
        returns:
            True on success OR
            False if the set was not changed OR
            -1 on error
        """
        try:
            ...
            if clear:
                ...
                if re.search(positivePattern, output):
                    ...
                    return False
            self.logger.error(self.name + ": setTestRemove did not" +
                              " match expected output")
            self.logger.debug(self.name + " expected: " + pattern)
            self.logger.debug(self.name + " actual: " + repr(output))
            return -1
        except AssertionError:
            self.logger.error(
    ...
    def setTestGet(self, setName, values=""):
        ...
        returns:
            ...
            A tuple containing the list then:
                False if the given values are not in the set OR
        ...
        try:
            ...
            if match:
                ...
                self.logger.error(self.name + ": setTestGet did not" +
                                  " match expected output")
                self.logger.debug(self.name + " expected: " + pattern)
                self.logger.debug(self.name + " actual: " + repr(output))
                return -1
        ...
        try:
            ...
            if match:
                ...
                if len(setMatch.split()) == setSize:
                    self.logger.info("The size returned by " + self.name +
                                     " matches the number of elements in " +
                                     "the returned set")
                else:
                    self.logger.error("The size returned by " + self.name +
...
#           main.exit()
    ...
    def counters(self, jsonFormat=True):
        ...
        try:
            ...
            self.logger.error("Error in processing 'counters' command: " +
                              str(output))
            return None
        except TypeError:
            self.logger.exception(self.name + ": Object not as expected")
...
#           main.exit()
    ...
    def summary(self, jsonFormat=True):
        ...
        Returns: json object ( summary -j ), returns False if there is
        no output
        ...
        try:
            ...
            if re.search("Error:", handle):
                self.logger.error(self.name + ": summary() response: " +
                                  str(handle))
            if not handle:
                self.logger.error(self.name + ": There is no output in " +
                                  "summary command")
                return False
            return handle
        except TypeError:
        ...
        try:
            ...
            try:
                ...
                self.logger.error("Error in processing '" + cmdStr + "' " +
                                  "command: " + str(output))
                return None
            pattern = "Key-value pair \(" + keyName + \
                ", (?P<value>.+)\) found."
        ...
        try:
            ...
            for line in output.splitlines():
                ...
                if new:
                    results[
                        new.groupdict()['key']] = {
                        'value': new.groupdict()['value']}
                elif updated:
                    results[
                        updated.groupdict()['key']] = {
                        'value': updated.groupdict()['value'],
