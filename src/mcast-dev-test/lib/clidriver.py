...
class CLI(object):
    ...
    def connect(self, **connectargs):
        ...
        self.handle.logfile = self.logger
        i = 5
        while i == 5:
            ...
            if i == 0:  # Accept key, then expect either a password prompt or access
                self.logger.info("ssh key confirmation received, send yes")
            ...
            if i == 1:  # Password required
                if self.pwd:
                    ...
                    self.logger.info("Server asked for password, but none was "
                ...
                self.logger.info("Password not required logged in")
    ...
    def execute(self, **execparams):
        ...
        if index == 0:
            ...
            self.logger.error("Expected Prompt not found , Time Out!!")
            self.logger.error(expectPrompt)
    ...
    def runAsSudoUser(self, handle, pwd, default):
        ...
        if i == 2:
            self.logger.error("Unable to run as Sudo user")
    ...
    def secureCopy(self, userName, ipAddress, filePath, dstPath, pwd="",
        ...
        if direction == "from":
            ...
            self.logger.debug("Wrong direction using secure copy command!")
            return False
        ...
        if i == 2:
            self.logger.info("Secure copy successful\n" + self.handle.before)
            returnVal = True
        elif i == 3:
            self.logger.error("Pexpect.EOF found!!!")
            # main.cleanup()
            # main.exit()
        elif i == 4:  # timeout
            self.logger.error(
